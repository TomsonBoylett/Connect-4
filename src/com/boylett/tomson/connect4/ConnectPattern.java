...
public class ConnectPattern {
    ...
    public static TreeNode<GameTreeValue> getMoveTree(TreeNode<GameTreeValue> currentNode, char[][] boardData, boolean turn, int depth) {
        if (depth == 0) {
            return currentNode;
        }
        int[][] possibleMoves = getPossibleMoves(boardData);
        for (int[] move : possibleMoves) {
            char[][] newBoardData =  new char[boardData.length][boardData[0].length];
            for (int i = 0; i < newBoardData.length; i++) {
                newBoardData[i] = Arrays.copyOf(boardData[i], boardData.length);
            }
            newBoardData[move[0]][move[1]] = ConnectGame.iconToChar(turn ? GameBoardSpace.RED : GameBoardSpace.YELLOW);
            TreeNode<GameTreeValue> newNode = new TreeNode<GameTreeValue>(new GameTreeValue(getHeuristic(newBoardData, turn), move), currentNode);
            getMoveTree(newNode, newBoardData, !turn, (depth - 1));
        }
        return currentNode;
    }
